...
class RPCAllocateFixedIP(object):
    ...
    def deallocate_fixed_ip(self, context, address, host=None, teardown=True):
        ...
        if network['multi_host'] and not utils.service_is_up(
                self.db.service_get_by_host_and_topic(context,
                                                      host,
                                                      'network')):
            # NOTE(vish): deallocate the fixed ip locally and don't teardown
            #             network devices
            super(RPCAllocateFixedIP, self).deallocate_fixed_ip(context,
                    address, teardown=False)
        else:
            topic = rpc.queue_get_for(context, FLAGS.network_topic, host)
            args = {'address': address,
                    'host': host}
...
class FloatingIP(object):
    ...
    def disassociate_floating_ip(self, context, address,
        ...
        if network['multi_host']:
            instance = self.db.instance_get_by_uuid(context,
                                                    fixed_ip['instance_uuid'])
            if utils.service_is_up(self.db.service_get_by_host_and_topic(
                ...
                #             not go over rpc and set interface to None so it the
...
class FlatManager(NetworkManager):
    ...
    def deallocate_fixed_ip(self, context, address, host=None, teardown=True):
        """Returns a fixed ip to the pool."""
        super(FlatManager, self).deallocate_fixed_ip(context, address, teardown)
        self.db.fixed_ip_disassociate(context, address)
